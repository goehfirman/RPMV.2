...
const App: React.FC = () => {
  ...
  const handleSubmit = async (data: FormData, apiKey: string) => {
    ...
    try {
      ...
      setRpmResult({ ...data, ...generatedContent });
    } catch (error: any) {
      console.error(error);
      alert("Terjadi kesalahan. Pastikan API Key Anda valid dan kuota mencukupi.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const WhatsAppButton = () => (
    ...
    >
      ...
      <span className="font-bold text-slate-700 text-xs">Pusat Bantuan</span>
    </a>
  );
        ...
        <div className="w-full max-w-sm animate-fade-in-up">
          <div className="bg-white border border-slate-200 p-8 rounded shadow-xl">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-2 uppercase tracking-widest">Kata Sandi</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-4 py-2.5 bg-slate-50 border rounded text-sm outline-none transition-all ${authError ? 'border-red-500 focus:border-red-600' : 'border-slate-300 focus:border-blue-600'}`}
                    placeholder="Masukkan sandi..."
                    autoFocus
                  />
                </div>
                {authError && <p className="text-red-500 text-[10px] mt-2 font-bold uppercase">Sandi Salah!</p>}
              </div>
              <button type="submit" className="w-full bg-[#1e3a8a] hover:bg-blue-900 text-white font-bold py-2.5 rounded shadow flex justify-center items-center gap-2 transition-all uppercase text-xs tracking-widest">
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <img src="https://i.ibb.co.com/1fQ81J6v/LOGO-PEKAYON-09.jpg" alt="Logo" className="w-10 h-10 object-contain" />
            <div>
              <h1 className="text-lg font-bold text-slate-800 tracking-tight flex items-center gap-2">RPM GENERATOR</h1>
        ...
        {!rpmResult ? (
          <div className="animate-fade-in-up">
             <div className="text-center space-y-4 mb-12">
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight uppercase">Sistem Perencanaan Pembelajaran</h2>
                <p className="text-slate-500 max-w-xl mx-auto text-sm font-medium">Otomasi penyusunan dokumen RPM berbasis kecerdasan buatan untuk efisiensi administrasi guru.</p>
             </div>
      ...
      <footer className="mt-20 py-10 border-t border-slate-200 bg-white no-print">
        <div className="max-w-7xl mx-auto px-4 text-center space-y-4">
          <p className="text-slate-500 text-[10px] font-bold tracking-[0.3em] uppercase">&copy; {new Date().getFullYear()} SDN PEKAYON 09 JAKARTA TIMUR</p>
          <p className="text-slate-400 text-[9px] font-medium tracking-wide uppercase">Dikembangkan secara mandiri untuk kemajuan pendidikan digital</p>
        </div>
      </footer>
      <WhatsAppButton />
    </div>
